...
MODEL_FILES = {
    ModelChoice.turbo: "ggml-large-v3-turbo.bin",
    ModelChoice.large_v3: "ggml-large-v3.bin",
}
...
def transcribe(
    ...
    model_path = models_dir / MODEL_FILES[model]

    if not model_path.exists():
        typer.echo(f"Model not found at {model_path}. Run scripts/setup.sh first.", err=True)
        raise typer.Exit(1)
